...
# KNN model
knnPred <- matrix("ans", 20, 10)
for(i in 1:10){knnPred[,i] <- as.character( knn(TrainCleaned2[,-1], TestCleaned2[,-1], TrainCleaned2$classe, k=i ) ) }
# problem id=8 has two possible answers(A or B)
...
# Fit a model in use of Random Forests
modelRF <- train(classe ~ ., data=TrainCleaned2, method="rf", prox=TRUE)
getTree( modelRF$finalModel, k=2)
predRF <- predict(modelRF, TestCleaned2[,-1])
...
svmTune <- train( x = TrainCleaned2[,-1],
                  y = TrainCleaned2[,1],
                  method = "svmRadial",
                  tuneLength = 9,
